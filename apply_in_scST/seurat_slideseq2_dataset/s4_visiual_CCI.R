...
load('./seurat_slideseq2_output.rda')
...
for (ct in celltype) {
  ...
  if(length(files)>0){
    ...
    LRTG_im_merge <- lapply(files, function(f){
      ...
      LRTG_im <- readRDS(paste0(inputdir,f))
      if(nrow(LRTG_im)==0){
        LRTG_im <- matrix(ncol = 7, dimnames = list(c(),c('LRpair','Ligand','Receptor','Target','IM','im_norm','Sender')))
      }else{
    ...
    LRTG_im_merge <- na.omit(LRTG_im_merge)
